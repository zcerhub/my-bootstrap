...
DEPLOY_DIR=`pwd`
SKYWALKING_DIR=$(dirname "$PWD")
SKYWALKING_SERVER_IP="10.64.103.81"
SKYWALKING_SERVER_PORT="12332"
...
SKYWALKING_JAR_PATH="${SKYWALKING_DIR}/skywalking-agent/skywalking-agent.jar"
JAVA_SKYWALKING_OPTS=" -javaagent:${SKYWALKING_JAR_PATH}=agent.service_name=dap_paas::${SERVER_NAME},agent.instance_name=${IP},collector.backend_service=${SKYWALKING_SERVER_IP}:${SKYWALKING_SERVER_PORT}"
echo -e "JAVA_SKYWALKING_OPTS ${JAVA_SKYWALKING_OPTS} ...\c"
echo -e "Starting the ${SERVER_NAME} ...\c"
#echo -e "nohup java ${JAVA_SKYWALKING_OPTS} ${JAVA_OPTS} ${JAVA_MEM_OPTS} ${JAVA_DEBUG_OPTS} ${JAVA_JMX_OPTS} -classpath ${CONF_DIR}:${LIB_JARS}:${FRONT_DIR} ${MAIN_CLASS} \c"
nohup java  ${JAVA_SKYWALKING_OPTS} ${JAVA_OPTS} ${JAVA_MEM_OPTS} ${JAVA_DEBUG_OPTS} ${JAVA_JMX_OPTS} -classpath ${CONF_DIR}:${LIB_JARS}:${FRONT_DIR} ${MAIN_CLASS} > ${STDOUT_FILE} 2>&1 &
