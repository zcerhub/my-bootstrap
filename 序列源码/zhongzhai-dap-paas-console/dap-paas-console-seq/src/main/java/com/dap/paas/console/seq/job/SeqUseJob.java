...
public class SeqUseJob implements BaseJob {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        ...
        SeqUseConditionDao seqUseConditionDao = SpringContext.getBeanNonnull(SeqUseConditionDao.class);
        // 保留三个月(90天数据)数据
        Map<String, Object> map = new HashMap<>();
        Date date = getDate(INT_90);
        LOGGER.warn("需要清理的时间截止日期为：{}", date);
        map.put("time", date);
        Integer deleteNum = seqUseConditionDao.deleteAttributeData(map);
        LOGGER.warn("清理使用记录条数：{}", deleteNum);
