...
class AuthenticationUserServiceImpl extends AbstractBaseServiceImpl<BaseSysUser, String> implements AuthenticationUserService {
    ...
    @Override
    public AuthenticationUserDto getUserByLoginName(String loginName,String tenantId,String authorization) {

        AuthenticationUserDto authenticationUserDTo = new AuthenticationUserDto();
        try {
            //得到用户名和密码
            BaseSysUser baseSysUser =baseSysUserService.getBaseSysUser(loginName,tenantId);
            if (null == baseSysUser) {
                return authenticationUserDTo;
            }
            authenticationUserDTo.setAuthorization((null==authorization|| "null".equalsIgnoreCase(authorization))?"":authorization);
    ...
    public AuthenticationUserDto getUserByLoginName(String loginName, String tenantId) {
        ...
        try {
            ...
            Map<String,Object> appMenuMap = UserInfoCache.get(SysConstant.All_APP_MENU);
            if(null == appMenuMap){
                return authenticationUserDTo;
            }

            Map<String,Object> operateMap = UserInfoCache.get(SysConstant.All_APP_OPERATE);

            Map<String,Object> allUrlsMap = UserInfoCache.get(SysConstant.All_APP_URLS);

            Map<String,Object>  operateSignMap= UserInfoCache.get(SysConstant.All_APP_OPERATE_SIGN);
