...
public enum ExceptionEnum implements IBaseError {
    ...
    SEQ_SERVER_CONNECT_FAIL("S2031", "序列服务端不可用!"),

    SEQ_GET_WORK_ID_FAIL("S2031", "当前应用未配置workId,且不能从序列服务端获取到workId,雪花算法无法使用！！！!"),

    SEQ_SNOW_INFO_EMPTY("S2032", "序列SDK，请求参数为空!"),

    SEQ_SNOW_PARAM_ERROR("S2033", "序列SDK，请求参数缺失!"),

    SEQ_SNOW_ADD_FAILURE("S2034", "序列服务端已无可用workId,获取失败!"),
    ...
    SEQ_INCREASE_REQUEST_ERROR("S2035", "严格递增序列请通过严格递增接口获取!"),

    SEQ_NOT_INCREASE_REQUEST_ERROR("S2036", "非严格递增序列请通过普通接口获取!"),

    SEQ_ASYNC_QUERY_FROM_SERVER_ERROR("S2037", "两段缓存均未从服务端获取到可用序列!"),

    SEQ_OPTIONAL_SEQ_VAL_ILLEGAL("S2038", "自选参数必须为纯数字!"),

    SEQ_OPTIONAL_REPEAT("S2039", "自选序号和普通序号重复!"),

    SEQ_RECOVERY_SERIAL_NOT_FOUND("S2040", "所回收的序号无法找到!"),

    CLOSE_STREAM_ERROR("S2038", "流关闭报错!"),
