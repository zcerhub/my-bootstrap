...
public class SysAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Authentication authentication) throws IOException, ServletException {
        try {
            ...
            result.setMsg("登录成功");
            result.setCode(200);
            authenticationUserDTo.setSessionId(httpServletRequest.getSession().getId());
            ...
            session.setAttribute(SysConstant.USER_ID,authenticationUserDTo.getUserId());

            result.setData(authenticationUserDTo);
