...
public class BaseContainerInfoController {
    ...
    public Result selectOne(String id) {
        try {
            BaseSysKubernetesInfo objectById = kubernetesInfoService.getObjectById(id);
            return ResultUtils.success(200, objectById);
        } catch (Exception e) {
            log.error("error", e);
            return ResultUtils.error(500, "查询异常");
        }
    ...
    public Result getK8sNamespaces(String agentId) {
        try {
            ...
            log.error("error", e);
            return ResultUtils.error(e.getStatusCode().value(), e.getMessage());
