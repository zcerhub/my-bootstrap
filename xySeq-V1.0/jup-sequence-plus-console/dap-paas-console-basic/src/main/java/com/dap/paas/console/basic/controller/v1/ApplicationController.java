...
import com.base.sys.api.common.UtilTools;
...
public class ApplicationController {
    ...
    public Result insert(@RequestBody Application application) {
        try {
            ...
            for(Application a:applicationList){
                if(application.getApplicationCode().equals(a.getApplicationCode()) || application.getApplicationName().equals(a.getApplicationName())){
                    return ResultUtils.error(ResultEnum.FAILED.getCode(),"已存在");
                }
            }
            //后台生成密钥和密钥启动状态（密钥用于应用调用openapi时访问机制校验）
    ...
    public Result queryPage(@RequestBody PageInDto<Application> param) {
        try {
            Page applications = applicationService.queryPage(param.getPageNo(),param.getPageSize(), param.getRequestObject());
    ...
    public Result update(@RequestBody Application application) {
        try {
            List<Application> applicationList = applicationService.queryList(new HashMap());
            for(Application a:applicationList){
                if((!a.getId().equals(application.getId())) && (application.getApplicationCode().equals(a.getApplicationCode()) || application.getApplicationName().equals(a.getApplicationName()))){
                    return ResultUtils.error(ResultEnum.FAILED.getCode(),"已存在");
                }
            }
